...
(async () => {
    ...
    let ort = null;
    try {
        ort = require(/* webpackIgnore: true */ 'onnxruntime-node-gpu');
    } catch (e1) {
        try {
            ort = require(/* webpackIgnore: true */ 'onnxruntime-node');
        } catch (e2) {
            console.error('onnxruntime-node{,-gpu} not found. Install dependency to enable inference.');
        }
    }
        const fs = require('fs');
        ...
        function train(cfg) {
            ...
            const py = spawn('python', ['./src/trainer.py', 'cfg.json']);
